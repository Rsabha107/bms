        ...
        <div class="card shadow-sm">
            <div class="card-body p-4 p-sm-5">
                <div class="text-center mb-7">
                    <h3 class="text-body-highlight"><a href="#">Edit User</a></h3>
                    <p class="text-muted">Edit account today</p>
                </div>
                ...
                <form method="POST" action="{{ route('sec.adminuser.update') }}" class="forms-sample" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="col-12 mt-4">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" id="userUser" type="radio" name="usertype" value="user" {{ $user->role == 'user' ? 'checked' : '' }} required>
                            <label class="form-check-label" for="userUser">User</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" id="adminUser" type="radio" name="usertype" value="admin" {{ $user->role == 'admin' ? 'checked' : '' }} required>
                    ...
                    <div class="col-12 gy-3 mb-3">
                        ...
                        <select class="form-select js-select-event-assign-multiple" id="add_event_assigned_to" name="event_id[]" multiple data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
                            @foreach ($events as $event)
                                <option value="{{ $event->id }}" {{ in_array($event->id, $user->events->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $event->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-12 gy-3 mb-3">
                        ...
                        <select class="form-select js-select-venue-assign-multiple" id="add_venue_assigned_to" name="venue_id[]" multiple data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
                            @foreach ($venues as $venue)
                                <option value="{{ $venue->id }}" {{ in_array($venue->id, $user->venues->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $venue->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- <div class="col-12 gy-3 mb-3">
                        ...
                        <select class="form-select js-select-fa-assign-multiple" id="add_fa_assigned_to" name="fa_id[]" multiple data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
                            @foreach ($functional_areas as $functional_area)
                                <option value="{{ $functional_area->id }}" {{ in_array($functional_area->id, $user->fa->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $functional_area->title }}</option>
                            @endforeach
                        </select>
                    </div> --}}
                    ...
                    <div class="col-sm-6 col-md-9 mb-3">
                        @foreach ($roles as $item)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" id="inlineCheckbox{{ $item->id }}" type="checkbox" name="roles[]" value="{{ $item->id }}" {{ $user->hasRole($item->name) ? 'checked' : '' }}>
