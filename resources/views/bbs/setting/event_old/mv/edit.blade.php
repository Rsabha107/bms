    ...
    <div class="text-center mb-3">
        <div class="mb-3 text-start">
            <input type="file" name="file_name" class="dropify" data-height="200"
                data-default-file="{{ !empty($event->event_logo) ? route('bbs.setting.event.file', $event->event_logo) : url('storage/upload/default.png') }}" />
                {{-- data-default-file="{{ !empty($event->event_logo) ? url('private/bbs/event/logo/' . $event->event_logo) : url('storage/upload/default.png') }}" /> --}}
        </div>
    </div>
