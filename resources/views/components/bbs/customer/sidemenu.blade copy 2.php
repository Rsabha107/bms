                ...
                <li class="nav-item">
                    ...
                    @foreach ($items as $index => $item)
                        @if ($item->children->isNotEmpty())
                            <div class="nav-item-wrapper">
                                <a class="nav-link dropdown-indicator label-1 collapsed" href="#nv-venue-services" role="button"
                                ...
                                <div class="parent-wrapper label-1">
                                    <ul class="nav collapse parent {{ session()->get($item->link) == 'active' ? 'show' : '' }}"
                                        ...
                                        {{-- first the 3 level child --}}
                                            @if ($child->children->isNotEmpty())
                                                <div class="nav-item-wrapper">
                                                    <a class="nav-link dropdown-indicator  label-1"
                                                        href="#nv-{{ $child->title }}-services" role="button"
                                                        data-bs-toggle="collapse" aria-expanded="false"
                                                        aria-controls="nv-{{ $child->title }}-services">
                                                        <div class="d-flex align-items-center">
                                                            ...
                                                            <span
                                                                class="nav-link-text text-wrap">{{ $child->title }}
                                                            </span>
                                                        </div>
                                                    </a>
                                                    <div class="parent-wrapper label-1">
                                                        <ul class="nav collapse parent {{ session()->get($child->link) == 'active' ? 'show' : '' }}"
                                                            data-bs-parent="#nv-venue-services"
                                                            id="nv-{{ $child->title }}-services">
                                                            <li class="collapsed-nav-item-title d-none">
                                                                {{ $child->title }}
                                                            </li>
                                                            @foreach ($child->children as $sub_child)
                                                                <li class="nav-item">
                                                                    <a class="nav-link {{ session()->get($sub_child->link) == 'active' ? 'active' : '' }}"
                                                                        href="{{ route('bbs.customer.booking.menu.show.services', $sub_child->id) }}">
                                                                        <div class="d-flex align-items-center"><span
                                                                                class="nav-link-text text-wrap">{{ $sub_child->title }}</span>
                                                                        </div>
                                                                    </a>
                                                                </li>
                                                            @endforeach
