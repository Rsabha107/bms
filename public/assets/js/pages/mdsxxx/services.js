$(document).ready(function () {
    ...
    $("body").on("click", "#offcanvas-add-service", function () {
        console.log("inside #offcanvas-add-project");
        // $("#add_edit_form").get(0).reset()
        // console.log(window.choices.removeActiveItems())
        $("#cover-spin").show();
        $("#offcanvas-add-service-modal").offcanvas("show");
        $("#cover-spin").hide();
    });

    $("body").on("click", "#edit_service_offcanv", function () {
        console.log("inside #edit_service_offcanv");
        $("#cover-spin").show();
        var id = $(this).data("id");
        ...
        $.ajax({
            url: "/bbs/setting/service/mv/get/" + id,
            method: "GET",
            async: true,
            success: function (response) {
                g_response = response.view;
                // console.log("g_response", g_response);
                $("#global-edit-service-content")
                    .empty("")
                    .append(g_response);
                $("#edit_schedule_table").val(table);
                $("#offcanvas-edit-service-modal").offcanvas("show");
                $("#cover-spin").hide();
            },
    ...
    $("body").on("click", "#editService", function () {
        ...
        $.ajax({
            url: "/bbs/setting/service/get/" + id,
            type: "get",
            headers: {
            ...
            success: function (response) {
                ...
                $("#edit_cargo_types_id").val(response.op.id);
                $("#edit_cargo_types_title").val(response.op.title);
...
$("body").on("click", "#deleteService", function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "/bbs/setting/service/delete/" + id,
                type: "DELETE",
                headers: {
                    "X-CSRF-TOKEN": $('input[name="_token"]').attr("value"), // Replace with your method of getting the CSRF token
                },
...
function actionsFormatter(value, row, index) {
    return [
        '<a href="javascript:void(0);" class="edit-service" id="edit_service_offcanv" data-id=' +
            row.id +
            " title=" +
            label_update +
            ' data-table="services_table" class="card-link"><i class="bx bx-edit mx-1"></i></a>' +
            "<button title=" +
            label_delete +
            ' type="button" data-table="services_table" class="btn delete" id="deleteService" data-id=' +
            row.id +
            ' data-type="status">' +
