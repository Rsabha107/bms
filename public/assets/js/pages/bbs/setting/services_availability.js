...
$("body").on("click", "#deleteServiceAvailability", function (e) {
    console.log("in deleteServiceAvailability");
    var id = $(this).data("id");
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "/bbs/match/service/availability/delete/" + id,
                type: "DELETE",
                headers: {
                    "X-CSRF-TOKEN": $('input[name="_token"]').attr("value"), // Replace with your method of getting the CSRF token
                },
...
function actionsFormatter(value, row, index) {
    return [
        '<a href="javascript:void(0);" class="edit-service" id="edit_match_service_availability_offcanvas" data-id=' +
            row.id +
            " title=" +
            label_update +
            ' data-table="services_availability_table" class="card-link"><i class="bx bx-edit mx-1"></i></a>' +
            "<button title=" +
            label_delete +
            ' type="button" data-table="services_availability_table" class="btn delete" id="deleteServiceAvailability" data-id=' +
            row.id +
            ' data-type="status">' +
